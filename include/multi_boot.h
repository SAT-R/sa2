 ...
 * From MASTER_REQUEST_CRC to MASTER_VERIFY_CRC are lower values
 * (Upper=0)
 */
...
//* Basically, SIO_INTR_FLAG of IE is cleared, however it is
// acceptable to use SIO interrupt.
//
...
//  However, if there is a wait long enough, it is acceptable to call
//  it multiple times per frame. The safe length of wait time depends
//  on the application. (It is necessary to make the wait longer than
//  the time it takes to process the interrupts of long period of
//  time, including V-Blank interrupts.)
//
//* Returns 0 in normal completion.
//
//* Returns 0 when transfer ends (successfully)  and a value of
// MultiBootCheckComplete() other than 0.
//
//* Once it is in the transfer end status, it simply returns 0
// immediately.
//  In order to cancel this status, call MultiBootInit().
//  It is acceptable to call MultiBootStartProbe() or
//  MultiBootStartMaster(), however, they are internally calling
//  MultiBootInit().
...
//  After calling this, the client's recognition processing is done
//  with        // MultiBootMain(). Call after verifying mp ->
//  probe_count is 0.
...
//                  For clients, it is downloaded immediately after
//                  the header.
...
//  "palette_color" and "palette_speed" are palette flash parameters
//  of client while loading. palette_color uses 0 to 6 to specify the
//  basic palettes (7 types) With "palette_speed", it flashes in
//  reversal order from -4 to -1 (-4 being the fastest) and it flashes
//  in normal from +1 to +4 (+4 being the fastest) Palette is fixed
//  (no flash) at 0. Do not specify any values other than listed
//  above.
...
//
//* Returns a value other than 0 if transfer has completed
//(successfully).
//  Returns 0 if it is still recognizing, has not started transfer, or
//  transfer has failed.
//
//* Calling any one of MultiBootInit(), MultiBootStartProbe() or
