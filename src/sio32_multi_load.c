    ...
    switch (gSio32MultiLoadArea.state) {
        case 0:
            if (gSio32MultiLoadArea.type ||
                gSio32MultiLoadArea
                    .frameCounter)  // Slave SIO Mode switch delay wait
        ...
        case 1:
            ...
            if (gSio32MultiLoadArea.type == SIO_SCK_IN) {         // Sets Master
                ...
                REG_SIOCNT |=
                    SIO_INTR_ENABLE | SIO_ENABLE;  // Starts communication
                REG_IME = 0;
        ...
        case 2:
            ...
            if (gSio32MultiLoadArea.type != SIO_SCK_IN) {  // Slave
                ...
                if (dataCounterBak > SIO32ML_BLOCK_SIZE / 4)
                    if ((gSio32MultiLoadArea.checkSum +=
                         gSio32MultiLoadArea.checkSumTmp) == -1)
                        gSio32MultiLoadArea.downloadSuccessFlag =
                            1;  // Succeeds download
            }
            if (dataCounterBak >
                    SIO32ML_BLOCK_SIZE / 4  // Complete communication
        ...
        case 3:
            ...
            *(vu64 *)REG_ADDR_SIODATA32 = 0;  // Clears data register
            ...
            REG_IF = INTR_FLAG_SERIAL | SIO32ML_TIMER_INTR_FLAG;  // Resets IF
            if (!gSio32MultiLoadArea.type) return 1;  // Slave comes back first
            gSio32MultiLoadArea.frameCounter = 0;
            gSio32MultiLoadArea.state = 4;
            break;
        case 4:
            if (gSio32MultiLoadArea.frameCounter >=
                3)  // Master comes back later
                return 1;
            break;
    }
    ++gSio32MultiLoadArea.frameCounter;
...
void Sio32MultiLoadIntr(void) {
    ...
    if (gSio32MultiLoadArea.type != SIO_SCK_IN) REG_SIOCNT |= SIO_ENABLE;
    if (gSio32MultiLoadArea.type == SIO_SCK_IN) {
        ...
        if (gSio32MultiLoadArea.dataCounter < 0)  // Sets synchronous data
            REG_SIODATA32 = SIO32ML_SYNC_DATA;
                 ...
                 SIO32ML_BLOCK_SIZE / 4)  // Sets sending data
            REG_SIODATA32 =
                gSio32MultiLoadArea.datap[gSio32MultiLoadArea.dataCounter];
        else
        ...
        if (gSio32MultiLoadArea.dataCounter < 0) {  // Checks synchronous data
            if (recvTmp != SIO32ML_SYNC_DATA) --gSio32MultiLoadArea.dataCounter;
        } else if (gSio32MultiLoadArea.dataCounter <
                   SIO32ML_BLOCK_SIZE / 4)  // Saves receiving data
            gSio32MultiLoadArea.datap[gSio32MultiLoadArea.dataCounter] =
                recvTmp;
        else
    ...
    if (gSio32MultiLoadArea.dataCounter < SIO32ML_BLOCK_SIZE / 4 + 3) {
        ...
        if (gSio32MultiLoadArea.type == SIO_SCK_IN) {
            ...
            REG_SIO32ML_TIMER_H = TIMER_1CLK | TIMER_INTR_ENABLE |
                                  TIMER_ENABLE;  // Restarts timer
        }
    }
}
...
void Sio32MultiLoadInit(u32 type, void *datap) {
    ...
    CpuCopy32(Sio32MultiLoadIntr, gMultiSioIntrFuncBuf,
              sizeof(gMultiSioIntrFuncBuf));
#endif
    *(vu32 *)REG_ADDR_SIOCNT =
        SIO_MULTI_MODE |
        MULTI_SIO_BAUD_RATE_NO;         // Stops multi-play communication
    gSio32MultiLoadArea.datap = datap;  // Sets data pointer
    gSio32MultiLoadArea.dataCounter = -1;
    if (type) {                 // Sets Master
        REG_SIO32ML_TIMER = 0;  // Resets timer
        gSio32MultiLoadArea.type = SIO_SCK_IN;
        for (i = 0; i < SIO32ML_BLOCK_SIZE / 4;
             ++i)  // Calculates sending data check sum
