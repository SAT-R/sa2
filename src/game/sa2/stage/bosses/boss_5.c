...
void sub_80436E4(EggSaucer *boss)
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        boss->unk10C[i][0] = Q_8_8(5.25) - (PSEUDO_RANDOM_32() & 255)
...
void sub_8043E2C(EggSaucer *boss)
{
    ...
    if (boss->gunHealth == 0 && boss->unk20 == 0) {
        ...
        if (Mod(gStageTime, 3) == 0) {
            ...
            explosion.rotation = ({ 1055 - (PSEUDO_RANDOM_32() & 63); });
...
static void sub_8044B28(EggSaucer *boss)
{
    ...
    if (++boss->unkBC >= 0x50) {
        if (boss->gunHealth != 0 || (PSEUDO_RANDOM_32() & 1)) {
            boss->unkBC = 0;
...
NONMATCH("asm/non_matching/game/sa2/stage/bosses/boss_5__sub_8045564.inc", void sub_8045564(EggSaucer *boss))
{
    ...
    if (boss->unk15 == 0) {
        ...
        if (Mod(gStageTime, 7) == 0) {
            rand = ({ (PSEUDO_RANDOM_32() & 0x1f) + 0x18; }) + 0;
            ...
            e.velocity = 0;
            e.rotation = ({ 0x407 - (PSEUDO_RANDOM_32() & 0x3F); }) + 0;
            e.speed = 0x500;
        ...
        if (Mod(gStageTime, 10) == 0) {
            ...
            e.spawnY += ((rand * SIN(index)) >> 14);
            e.velocity = 0x40;
            e.rotation = ({ (PSEUDO_RANDOM_32() & 0x3FF); }) + 0;
            e.speed = 0x600;
        ...
        if (Mod(gStageTime, 9) == 0) {
            ...
            e.rotation = ({ 0x407 - (PSEUDO_RANDOM_32() & 63); }) + 0;
            e.speed = 0x500;
            e.vram = (void *)OBJ_VRAM0 + 0x2980;
            e.anim = SA2_ANIM_EXPLOSION;
            e.variant = 0;
