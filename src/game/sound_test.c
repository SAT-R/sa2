...
static void SoundTestScreenRenderUI(void)
{
    ...
    if (soundTestScreen->state == SOUND_TEST_SCREEN_PLAYING) {
        ...
        for (i = 0; i < 8; i++) {
            if (soundTestScreen->channelFreqs[i] != soundInfo->chans[i].frequency) {
                s32 change = soundTestScreen->channelFreqs[i] - soundInfo->chans[i].frequency;
                ...
                soundTestScreen->channelFreqs[i] = soundInfo->chans[i].frequency;
            }
        }
