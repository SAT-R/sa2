...
void Task_RingsMgrMain(void)
{
    ...
    if (!(gStageFlags & STAGE_FLAG__2)) {
        ...
        rings = *(u32 **)((uintptr_t)TASK_DATA(gCurTask) + offsetof(RingsManager, rings));
            ...
            for (i = 0; i < MULTI_SIO_PLAYERS_MAX; i++) {
                ...
                if (i != id && gMultiplayerPlayerTasks[i] != NULL) {
                    ...
                    for (regionY = REGION_LOWER(mpp->pos.y, mpp->s.hitboxes[0].top, 0);
                         regionY <= REGION_UPPER(mpp->pos.y, mpp->s.hitboxes[0].bottom, 8) && regionY < v_regionCount; regionY++) {

                        for (regionX = REGION_LOWER(mpp->pos.x, mpp->s.hitboxes[0].left, -8);
                             regionX <= REGION_UPPER(mpp->pos.x, mpp->s.hitboxes[0].right, 16) && regionX < h_regionCount; regionX++) {
...
#else
            for (; TO_WORLD_POS(0, regionY) < gCamera.x + DISPLAY_HEIGHT && regionY < v_regionCount; regionY++) {
#endif
