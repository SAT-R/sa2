...
static inline uint16_t BlendSpritePixel(
    uint16_t color, unsigned int x,
    uint16_t *output, uint8_t *layerIds,
    bool isSemiTransparent,
    unsigned int blendMode, unsigned int bldcnt,
    bool windowsEnabled, uint16_t *winMask,
    unsigned int eva, unsigned int evb, unsigned int evy)
{
    bool winAllowsBlend = !windowsEnabled
        || (winMask && (winMask[x] & WINMASK_CLR));

    bool doAlpha = (blendMode == 1 && (bldcnt & BLDCNT_TGT1_OBJ) && winAllowsBlend)
                   || isSemiTransparent;
...
static inline void WriteBGPixelBlended(
    unsigned int x, uint8_t pixel,
    const uint16_t *palBase, int bgNum,
    uint16_t *output, uint8_t *layerIds,
    unsigned int blendMode, bool bgIsTargetA,
    bool useWindows, unsigned int winBgBit, uint16_t *winMask,
    unsigned int bldcnt,
    unsigned int eva, unsigned int evb, unsigned int evy)
{
    ...
    if (bgIsTargetA && (!useWindows || (winMask[x] & WINMASK_CLR))) {
        ...
        switch (blendMode) {
            case 1:
                ...
                break;
            case 2: color = alphaBrightnessIncrease(src, evy); break;
            case 3: color = alphaBrightnessDecrease(src, evy); break;
        }
    }
...
static void RenderTextBG(int bgNum, uint16_t control,
                         uint16_t hoffs, uint16_t voffs,
                         int lineNum, uint16_t *output)
{
    ...
    if (hasMosaic || is8bpp) {
        for (unsigned int x = 0; x < DISPLAY_WIDTH; x++) {
            unsigned int xx = hasMosaic
                ? (ApplyMosaicBGX(x) + hoffs) & wMask
                : (x + hoffs) & wMask;
            ...
            if (TILE_HFLIP(entry)) tx = 7 - tx;
            if (TILE_VFLIP(entry)) ty = 7 - ty;
    ...
    // left edge: partial tile if scroll isn't tile-aligned
    {
        ...
        if (startOff != 0) {
            ...
            if (partial > DISPLAY_WIDTH) partial = DISPLAY_WIDTH;
    ...
    while (x + 8 <= DISPLAY_WIDTH) {
        ...
        if (!TILE_HFLIP(entry)) {
            uint8_t p;
            p = row & 0xF;          if (p) output[x  ] = palBase[p] | COLOR_OPAQUE;
            p = (row >>  4) & 0xF;  if (p) output[x+1] = palBase[p] | COLOR_OPAQUE;
            p = (row >>  8) & 0xF;  if (p) output[x+2] = palBase[p] | COLOR_OPAQUE;
            p = (row >> 12) & 0xF;  if (p) output[x+3] = palBase[p] | COLOR_OPAQUE;
            p = (row >> 16) & 0xF;  if (p) output[x+4] = palBase[p] | COLOR_OPAQUE;
            p = (row >> 20) & 0xF;  if (p) output[x+5] = palBase[p] | COLOR_OPAQUE;
            p = (row >> 24) & 0xF;  if (p) output[x+6] = palBase[p] | COLOR_OPAQUE;
            p = (row >> 28) & 0xF;  if (p) output[x+7] = palBase[p] | COLOR_OPAQUE;
        } else {
            uint8_t p;
            p = (row >> 28) & 0xF;  if (p) output[x  ] = palBase[p] | COLOR_OPAQUE;
            p = (row >> 24) & 0xF;  if (p) output[x+1] = palBase[p] | COLOR_OPAQUE;
            p = (row >> 20) & 0xF;  if (p) output[x+2] = palBase[p] | COLOR_OPAQUE;
            p = (row >> 16) & 0xF;  if (p) output[x+3] = palBase[p] | COLOR_OPAQUE;
            p = (row >> 12) & 0xF;  if (p) output[x+4] = palBase[p] | COLOR_OPAQUE;
            p = (row >>  8) & 0xF;  if (p) output[x+5] = palBase[p] | COLOR_OPAQUE;
            p = (row >>  4) & 0xF;  if (p) output[x+6] = palBase[p] | COLOR_OPAQUE;
            p = row & 0xF;          if (p) output[x+7] = palBase[p] | COLOR_OPAQUE;
        }
        x += 8;
    }
...
static void RenderTextBGBlend(int bgNum, uint16_t control,
                              uint16_t hoffs, uint16_t voffs,
                              int lineNum, uint16_t *output,
                              uint8_t *layerIds,
                              ...
                              unsigned int evy)
{
    ...
    if (hasMosaic || is8bpp) {
        for (unsigned int x = 0; x < DISPLAY_WIDTH; x++) {
            ...
            uint16_t color = !is8bpp
                ? pal[(palNum << 4) + pixel] | COLOR_OPAQUE
                : pal[pixel] | COLOR_OPAQUE;
    ...
    // left edge partial tile
    {
        ...
        if (startOff != 0) {
            ...
            for (unsigned int t = 0; t < partial && x < DISPLAY_WIDTH; t++, x++) {
                ...
                uint8_t pixel = (row >> (tx << 2)) & 0xF;
                if (pixel)
                    WriteBGPixelBlended(x, pixel, palBase, bgNum, output,
                                        layerIds, blendMode, bgIsTargetA,
                                        useWindows, winBgBit, winMask,
                                        bldcnt, eva, evb, evy);
            }
        }
    }
    ...
    while (x + 8 <= DISPLAY_WIDTH) {
        ...
        #define BLEND_PX(off, shift) do {                                \
            uint8_t p = (row >> (shift)) & 0xF;                          \
            if (p) WriteBGPixelBlended(x+(off), p, palBase, bgNum,       \
                       output, layerIds, blendMode, bgIsTargetA,         \
                       useWindows, winBgBit, winMask,                    \
                       bldcnt, eva, evb, evy);                           \
        } while (0)

        if (!TILE_HFLIP(entry)) {
            BLEND_PX(0,  0); BLEND_PX(1,  4);
            BLEND_PX(2,  8); BLEND_PX(3, 12);
            BLEND_PX(4, 16); BLEND_PX(5, 20);
            BLEND_PX(6, 24); BLEND_PX(7, 28);
        } else {
            BLEND_PX(0, 28); BLEND_PX(1, 24);
            BLEND_PX(2, 20); BLEND_PX(3, 16);
            BLEND_PX(4, 12); BLEND_PX(5,  8);
            BLEND_PX(6,  4); BLEND_PX(7,  0);
        }
...
static void RenderAffineBG(int bgNum, uint16_t control,
                           int lineNum, uint16_t *output)
{
...
static void RenderAffineBGBlend(int bgNum, uint16_t control,
                                int lineNum, uint16_t *output,
                                uint8_t *layerIds,
                                unsigned int blendMode,
                                bool windowsEnabled,
...
#if !EXTENDED_OAM
        if (sx >= DISPLAY_WIDTH)  sx -= 512;
        if (sy >= DISPLAY_HEIGHT) sy -= 256;
#endif
...
static void DrawSpritesAtPriority(int priority, uint16_t vcount,
                                  uint16_t *output, uint8_t *layerIds,
                                  bool windowsEnabled, uint16_t *winMask,
                                  unsigned int blendMode, bool objWinOnly,
                                  unsigned int bldcnt,
                                  unsigned int eva, unsigned int evb,
                                  unsigned int evy)
{
    ...
    for (int s = 0; s < sActiveSpriteCount[priority]; s++) {
        ...
        if (objWinOnly && !isObjWin) continue;
        if (!objWinOnly && isObjWin) continue;
        ...
        int localY = (oam->split.mosaic == 1)
                      ? ApplyMosaicSprY(vcount) - y
                      : vcount - y;
        ...
        if (!isAffine && !is8bpp && !oam->split.mosaic) {
            ...
            while (lx <= endLX) {
                ...
                uint32_t rowData = *(uint32_t *)(tiledata
                    + ((tileBase + blockX) << 5) + rowByteOff);

                int pixelsInTile = !flipX ? (8 - tileXStart) : (tileXStart + 1);
                int remain = endLX - lx + 1;
                if (pixelsInTile > remain) pixelsInTile = remain;

                if (!flipX) {
                    int texRemain = (int)width - texX;
                    if (pixelsInTile > texRemain) pixelsInTile = texRemain;
                } else {
                ...
                for (int p = 0; p < pixelsInTile; p++, lx++) {
                    ...
                    if (layerIds && blendMode != 0)
                        color = BlendSpritePixel(color, gx, output, layerIds,
                                                 isSemiTransparent, blendMode,
                                                 bldcnt, windowsEnabled, winMask,
                                                 eva, evb, evy);
...
static void DrawScanline(uint16_t *pixels, uint16_t vcount)
{
    ...
    if (windowsEnabled && (REG_DISPCNT & DISPCNT_OBJWIN_ON) && (REG_DISPCNT & DISPCNT_OBJ_ON)) {
        for (int pri = 0; pri < 4; pri++)
            DrawSpritesAtPriority(pri, vcount, pixels, lids,
                                  windowsEnabled, wmask, blendMode,
                                  /*objWinOnly=*/true,
    ...
    for (int pri = 3; pri >= 0; pri--) {
        for (int sub = bgsByPriCount[pri] - 1; sub >= 0; sub--) {
            ...
            if (!needLayerIds) {
                switch (mode) {
                    case 0:
                        RenderTextBG(bg, bgcnts[bg],
                            *(uint16_t *)(REG_ADDR_BG0HOFS + bg * 4),
                            *(uint16_t *)(REG_ADDR_BG0VOFS + bg * 4),
                            vcount, pixels);
                        break;
                    case 1:
                        if (bg == 2)
                ...
                switch (mode) {
                    case 0:
                        RenderTextBGBlend(bg, bgcnts[bg],
                            *(uint16_t *)(REG_ADDR_BG0HOFS + bg * 4),
                            *(uint16_t *)(REG_ADDR_BG0VOFS + bg * 4),
                            vcount, pixels, lids, blendMode,
                            windowsEnabled, wmask,
                            bldcnt, bld_eva, bld_evb, bld_evy);
                        break;
                    case 1:
                        if (bg == 2)
                            RenderAffineBGBlend(bg, bgcnts[bg],
                                vcount, pixels, lids, blendMode,
                                windowsEnabled, wmask,
                                bldcnt, bld_eva, bld_evb, bld_evy);
                        else
